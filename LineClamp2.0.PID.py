...
def checkRed(data):
    if ((data[0] < (data[1]*2.5)) & (data[4] < (data[5]*2.5)) &
...
def driveForward(hiVals, loVals, topSpeed, color='clear'):
    ...
    eTotal = elapsed
    iSawRed = checkRed(data)
    if checkLost(data) = True:
        break
    print(data[c+12], data[c+8], data[c+4], data[c])
    i = 1
    elapsed = 0
    ...
    while iSawRed = False:
        ...
        if data[c] > (data[c+4]*2): # Too far left; make a hard right!
            print('Hard right!')
            integral = 0
            while data[c+4] < (hiVals * .8):
                pi.set_PWM_dutycycle(M2Sp, 0)
                pi.set_PWM_dutycycle(M1Sp, (topSpeed * .65))
                start = time.time()
                data = getSensorData()
                end = time.time()
                elapsed = round((end-start), 5)
            ...
            while data[c+8] < (hiVals * .8):
                pi.set_PWM_dutycycle(M2Sp, (topSpeed * .65))
                pi.set_PWM_dutycycle(M1Sp, 0)
                start = time.time()
                data = getSensorData()
                end = time.time()
            ...
            print('Correction: ' + str(round((correction), 0)) +
                  ' / E: ' + str(error) +
                  ' / P: ' + str(round((proportional*kp), 0)) +
                  ' / I: ' + str(round((integral*ki), 0)) +
                  ' / D: ' + str(round((derivative*kd), 0)) +
                  ' / T: ' + str(round((turn),0)))
...
topSpeed = 150  # Speed values range from 0-255.
hiV = 300
