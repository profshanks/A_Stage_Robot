...
'''
Sam hacked this script to make it work with the pigpio library. This allows for remote-SSH
control of the sensors over WiFi. Neat! (I broke it for use with Circuit Python etc.)
'''
    ...
    **Quickstart: Importing and using the device**
    ...
    def _transceive(self, payload, rxlen=0):
        ...
        if len(payload) == 1:
            ...
            print(payload)
            print()
            self.pi.i2c_write_device(self.i2c_device, payload)  # _pigpio
                
                
        else:
            # Write 'value' to the specified register
            # TODO: Check this. It's weird that the write is accepted but the read is naked.
            print("Remember that 'else' condition you were hoping would never come up...")
            '''
            with self.i2c_device as i2c:
                i2c.write(payload)
    ...
    def _set_resolution(self, axis, resolution):
        ...
        self.pi.i2c_write_byte_data(self.i2c_device,
                                    reg, 0x02)  # _pigpio
    ...
    def reset(self):
        ...
        status = self.pi.i2c_zip(self.i2c_device,
                                 [7, 1, 0xf0])
        #self.pi.i2c_write_device(self.i2c_device, bytes([_CMD_RT]))  # _pigpio
    ...
    def read_data(self):
        ...
        combinedCommand = 0x3e
        print(combinedCommand)
        status = self.pi.i2c_zip(self.i2c_device,
                                 [7, 1, combinedCommand, 6, 1])   # _pigpio
        print(status)
