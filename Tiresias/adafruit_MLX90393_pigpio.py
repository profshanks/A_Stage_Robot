...
* Author(s): ktown, profshanks(ported to work with pigpio, Feb 2022)
...
'''
profshanks hacked this script to make it work with the pigpio library. This allows
for remote-SSH control of the sensors over WiFi. Neat! (I broke it for use
with Circuit Python etc.)
'''
...
class MLX90393:  # pylint: disable=too-many-instance-attributes
    ...
    **Quickstart: Importing and using the device**
    ...
    def __init__(
        ...
        self.pi = pi # _pigpio
        self.i2c_device = self.pi.i2c_open(1, address) # _pigpio
        self._debug = debug
        self._status_last = 0
    ...
    def gain(self, value):
        ...
        register = int(_CMD_REG_CONF1)
        register = register << 2   # Shift register num left by 2 as per datasheet
        
        # Read contents of register 2: Oversampling, Filter, & Resolution
        readout = self.pi.i2c_zip(self.i2c_device,
                                [7, 2, 0x50, register, 6, 3])

        reg = readout[1]   # Just the bytearray please
        reg = reg[1:]      # Slice off the status byte from the front
        reg = int.from_bytes(reg, byteorder='big')   # Convert to int
        reg &= 0xFF80   # Mask off GAIN bits

        data = self._gain_current << _GAIN_SHIFT | _HALLCONF
        reg |= data  # Apply GAIN settings

        #Write 'reg' to register '_CMD_REG_CONF3'
        regbytes = reg.to_bytes(2, 'big')  # Convert back to bytearray
        a = regbytes[0]  # Separate the two bytes
        b = regbytes[1]
        status = self.pi.i2c_zip(self.i2c_device,
                        [7, 4, 0x60, a, b, register, 6, 1])
    @property
    ...
    def _set_resolution(self, axis, resolution):
        ...
        reg = reg[1:]      # Slice off the status byte from the front
        reg = int.from_bytes(reg, byteorder='big')   # Convert to int
        reg &= mask   # Mask off the part we care about
        reg |= (resolution & 0x3) << shift   # Apply resolution setting
    ...
    def filter(self, level):
        ...
        reg = reg[1:]      # Slice off the status byte from the front
        reg = int.from_bytes(reg, byteorder='big')   # Convert to int
        reg &= 0xFFE3   # Mask off filter bits
        reg |= (level & 0x7) << 2   # Apply filter settings
    ...
    def oversampling(self, level):
        ...
        reg = reg[1:]      # Slice off the status byte from the front
        reg = int.from_bytes(reg, byteorder='big')   # Convert to int
        reg &= 0xFFEc   # Mask off oversampling bits
        reg |= level & 0x3   # Apply oversampling settings
    ...
    def read_data(self):
        ...
        SM_Command = 0x3e
        status = self.pi.i2c_zip(self.i2c_device,
                                 [7, 1, SM_Command, 6, 1])   # _pigpio
        '''
        # Optional code for reading status byte & publishing results
        statusbits = status[1]
        self._status_last = statusbits[0]
        self.display_status()
        '''
        ...
        # Read the 'XYZ' data
        data = self.pi.i2c_zip(self.i2c_device,
                                 [7, 1, 0x4e, 6, 7])   # _pigpio
